...
function renderBlogPosts(items) {
  ...
  items.slice(0, POST_COUNT).forEach(item => {
    ...
    excerpt.textContent = truncate(htmlToText(item.description || ''), EXCERPT_LENGTH);
...
fetch(RSS2JSON_URL)
  ...
  .catch(() => {
    ...
    if (blogStatus) {
      blogStatus.textContent = "Couldn't load latest posts — see them directly on Medium below.";
    }
  });
